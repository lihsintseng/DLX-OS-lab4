...
#define FILE_MAX_FILENAME_LENGTH2 72
...
int TrapFileOpenHandler(uint32 *trapArgs, int sysMode) {
  char filename[FILE_MAX_FILENAME_LENGTH2];
  ...
  if (!sysMode) {
    ...
    dstrncpy(filename, (char *)(trapArgs[0]), FILE_MAX_FILENAME_LENGTH2);
...
//	FileMkdirHandler
//
//	Trap handler for mkdir(char *path)
//-----------------------------------------------------------------------
//mkdir(char *path, int permissions)
int TrapFileMkDirHandler(uint32 *trapArgs, int sysMode) {
  char filename[FILE_MAX_FILENAME_LENGTH2];
  char *user_filename = NULL;
  //char mode[10];
  int permissions;
  ...
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem (currentPCB, (trapArgs+0), &user_filename, sizeof(uint32));
    // Argument 1: userland pointer to mode string
    MemoryCopyUserToSystem (currentPCB, (trapArgs+1), &permissions, sizeof(uint32));
    ...
    if (i == FILE_MAX_FILENAME_LENGTH2) {
      printf("TrapFileMkDirHandler: length of filename longer than allowed!\n");
      GracefulExit();
    }
    dbprintf('F', "TrapFileMkDirHandler: just parsed filename (%s) from trapArgs\n", filename);
 
    
    dbprintf('F', "TrapFileMkDirHandler: just parsed permissions (%d) from trapArgs\n", permissions);
  } else {
    // Already in kernel space, no address translation necessary
    dstrncpy(filename, (char *)(trapArgs[0]), FILE_MAX_FILENAME_LENGTH2);
    permissions = trapArgs[1];
  }
  dbprintf('F', "TrapFileMkDirHandler: calling MkDir(\"%s\", \"%d\")\n", filename, permissions);
  return MkDir(filename, permissions);
}

int TrapLinkHandler(uint32 *trapArgs, int sysMode) {
  char srcpath[FILE_MAX_FILENAME_LENGTH2];
  char dstpath[FILE_MAX_FILENAME_LENGTH2];
  char *user_srcpath = NULL;
  char *user_dstpath = NULL;
  int i;
  // If we're not in system mode, we need to copy everything from the
  // user-space virtual address to the kernel space address
  if (!sysMode) {
    // Get the arguments themselves into system space
    // Argument 0: userland pointer to srcpath string
    MemoryCopyUserToSystem (currentPCB, (trapArgs+0), &user_srcpath, sizeof(uint32));
    // Argument 1: userland pointer to dstpath string
    MemoryCopyUserToSystem (currentPCB, (trapArgs+1), &user_dstpath, sizeof(uint32));

    // Now copy userland filename string into our filename buffer
    for(i=0; i<FILE_MAX_FILENAME_LENGTH2; i++) {
      MemoryCopyUserToSystem(currentPCB, (user_srcpath+i), &(srcpath[i]), sizeof(char));
      // Check for end of user-space string
      if (srcpath[i] == '\0') break;
    }
    if (i == FILE_MAX_FILENAME_LENGTH2) {
      printf("TrapFileMkDirHandler: length of srcpath longer than allowed!\n");
      GracefulExit();
    }
    for(i=0; i<FILE_MAX_FILENAME_LENGTH2; i++) {
      MemoryCopyUserToSystem(currentPCB, (user_dstpath+i), &(dstpath[i]), sizeof(char));
      // Check for end of user-space string
      if (dstpath[i] == '\0') break;
    }
    if (i == FILE_MAX_FILENAME_LENGTH2) {
      printf("TrapFileMkDirHandler: length of dstpath longer than allowed!\n");
      GracefulExit();
    }
    dbprintf('F', "TrapFileMkDirHandler: just parsed  (%s) from trapArgs\n", srcpath);
    dbprintf('F', "TrapFileMkDirHandler: just parsed  (%s) from trapArgs\n", dstpath);
  } else {
    // Already in kernel space, no address translation necessary
    dstrncpy(srcpath, (char *)(trapArgs[0]), FILE_MAX_FILENAME_LENGTH2);
    dstrncpy(dstpath, (char *)(trapArgs[1]), FILE_MAX_FILENAME_LENGTH2);
  }
  return FileLink(srcpath, dstpath);
}
//-----------------------------------------------------------------------
//	fileRmdirHandler
//
//	Trap handler for rmdir(char *path)
//-----------------------------------------------------------------------
//rmdir(char *path)
int TrapFileRmDirHandler(uint32 *trapArgs, int sysMode) {
  char filename[FILE_MAX_FILENAME_LENGTH2];
  char *user_filename = NULL;
  int i;
  ...
  if (!sysMode) {
    ...
    if (i == FILE_MAX_FILENAME_LENGTH2) {
      printf("TrapFileRmDirHandler: length of filename longer than allowed!\n");
      GracefulExit();
    }
    dbprintf('F', "TrapFileRmDirHandler: just parsed filename (%s) from trapArgs\n", filename);
 
  } else {
    // Already in kernel space, no address translation necessary
    dstrncpy(filename, (char *)(trapArgs[0]), FILE_MAX_FILENAME_LENGTH2);    
  }
  dbprintf('F', "TrapFileRmDirHandler: calling RmDir(\"%s\")\n", filename);
	     ...
	     uint32 *trapArgs)
{
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    // Traps for file and directory functions
    case TRAP_FILE_OPEN:
    ...
    case TRAP_FILE_MKDIR:
        ProcessSetResult(currentPCB, TrapFileMkDirHandler(trapArgs, isr & DLX_STATUS_SYSMODE));
      break;
    case TRAP_FILE_RMDIR:
        ProcessSetResult(currentPCB, TrapFileRmDirHandler(trapArgs, isr & DLX_STATUS_SYSMODE));
      break;
    case TRAP_LINK:
        ProcessSetResult(currentPCB, TrapLinkHandler(trapArgs, isr & DLX_STATUS_SYSMODE));
      break;
