  ...
  for(i = 0; i < sb.number_of_inodes; i++){
    for(k = 0; k < 96; k++){
      if(k == 0) inodes[i].inuse = b->data[j++];
        ...
        inodes[i].filename[k-9] = b->data[j++];
      else if(k > 47 && k <= 87)
...
//      b.data[j] = inode[i][k];
      if(k == 0) b.data[j] = inodes[i].inuse;
        ...
        b.data[j] = inodes[i].filename[(k-9)];
      else if(k > 47 && k <= 87)
...
//-----------------------------------------------------------------
// DfsInodeFilenameExists looks tough all the inuse inodes for 
...
int DfsInodeFilenameExists(char *filename) {
  int i = 0;
//  printf("here, number of inodes: %d\n", sb.number_of_inodes);
  for(i = 0 ; i < sb.number_of_inodes; i++){
//    printf("%d: inuse: %d match %d name: %s, compare_name: %s\n ", i, inodes[i].inuse, matchfilename(inodes[i].filename, filename), inodes[i].filename, filename);
    if((inodes[i].inuse != 0) && (matchfilename(inodes[i].filename, filename) != 0) ){
    //printf("%d: inuse: %d match %d name: %s, compare_name: %s\n ", i, inodes[i].inuse, matchfilename(inodes[i].filename, filename), inodes[i].filename, filename);
      return i;
    }
  }
...
int DfsInodeOpen(char *filename) {
  uint32 inode_index = DfsInodeFilenameExists(filename);
  int i = 0;
  int c = 0;
//  printf("DfsInodeOpen: found the file: %d\n", inode_index);
  if(inode_index == DFS_FAIL){
    for(i = 0; i < sb.number_of_inodes; i++){
      if(inodes[i].inuse == 0){
        break;
      }
    ...
    for(c = 0; c < filename_size; c++){
      inodes[i].filename[c] = filename[c];
    }
    inodes[i].size = 0;
    inodes[i].bsize = 0;

    //printf("DfsInodeOpen: found the file: %s\n", inodes[i].filename);
    ...
    //printf("The write data exist, clean it %d\n", inode_index);
    DfsInodeDelete(inode_index);
    LockHandleRelease(inodelock);
    return DfsInodeOpen(filename);
...
//    printf("Allocate direct addr : %d to addr %d\n", virtual_blocknum, free_addr);
    ...
    virtual_blocknum -= 10;
//    printf("Allocate indirect addr : %d, %d\n", virtual_blocknum, nowsize);
    if(nowsize == 10){
...
uint32 matchfilename(char* filename, char * comparename){
  ...
  for(i = 0; i < filename_size; i++){
    if(comparename[i] == '\0' && filename[i] == '\0') return 1;
